...
async def test_create_favorite_list():
    resp = await favorite_list.create_video_favorite_list(
        title="Test Favorite List",
        introduction="This is a test favorite list.",
        private=True,
        credential=cre
    )
    id = resp.get("id")
...
async def test_get_favorite_list():
    resp = await favorite_list.get_video_favorite_list(
        uid=uid,
        credential=cre
    )
...
async def test_delete_favorite_list():
    resp = await favorite_list.delete_video_favorite_list(
        media_ids=[3533640315],
        credential=cre
    )
